...
桌面小工具：每天早上 8:30 自动弹窗让你选/输入上班时间，下班前 2 分钟自动启动并弹窗提醒。无需常驻后台。
...
1. 双击 [安装计划任务.bat](安装计划任务.bat)
   - **解锁触发**（`OffReminder_Unlock`）：你解锁工作站时自动弹窗，预填当前时间作为上班时间（当天已记录过则静默退出）
   - **兑底**（`OffReminder_Morning`）：每天 **08:30** 弹一次，防止你没锁屏、或开机后未解锁、或全天未弹出过
...
```
解锁屏幕  ── 启动程序 ── 弹窗选择/输入上班时间（预填当前时间）
   │                                          │
   └─ 当天已记录 → 静默退出                       └─ 自动注册一次性任务（下班前 2 分钟）
08:30  兑底 ── 如果当天还未弹出过，同上弹窗          │
                                                          │
下班-2分钟  系统调度 ── 启动程序 ── 弹窗 + 响铃 提醒下班 ── 关闭提醒即退出程序
```
...
| `PROMPT_TIME` | 早上提示时间（程序内倒计时用） | (8, 30) |
| `PRESET_START_TIMES` | 预设上班时间按钮 | 8:20 / 8:25 / 8:30 |

> 改了早上时间后需重跑 [安装计划任务.bat](安装计划任务.bat)（任务计划写死 08:30），或在「任务计划程序」里手工调整 `OffReminder_Morning` 触发器。
...
- `OffReminder_Morning` — 每天 08:30
- `OffReminder_Unlock` — 解锁工作站时触发
